#!/usr/bin/env python2
#
# Original: I, Danny Milosavljevic, hereby place this file into
#           the public domain.

# Implementation of the C64 CIA6526 chips.
# resources located at https://www.c64-wiki.com/index.php/CIA
...
class CIA1(memory.Memory):
    ...
    def __init__(self):
        ...
        self.joysticks = [joysticks.DigitalJoystick(),
                          joysticks.DigitalJoystick()]
    ...
    def read_memory(self, address, size=1):
        ...
        if address == A_KEYBOARD_MATRIX_JOYSTICK_2:
            #if (self.A_data_direction & 31) == 0:
            # sets the joystick bits to "read".
            #TODO could also be just one of them?
            ...
            if self.keyboard_matrix_rows != 0:  # is not None:
                ...
                for row in range(0, 8):
                    ...
                    if (self.keyboard_matrix_rows & (1 << row)) != 0:
                        columns = CIA1.matrix[row]
                        # print("possible", rows)
                        for column_i, cell in enumerate(columns):
                            if cell in self.pressed_keys:
                                # or (isinstance(cell, int) and cell < 128 and
                                # (cell | 0x20) in self.pressed_keys):
                                #print("YESSS, matched", cell)
                                v |= (1 << column_i)
                #print("INVKEY", v)
                return 255 - v

            # return bits cleared in rows where a key is pressed
            # in self.keyboard_matrix_column.
            return 0xFF  # nothing.
...
class CIA2(memory.Memory):
    ...
    def read_memory(self, address, size=1):
        ...
        return {
            B_PORT_A_SERIAL: (3 - self.VIC_bank) +
                             self.serial.get_control_mask(),
